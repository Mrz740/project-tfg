...
func _change_scene_with_led_sync(scene_path: String) -> void:
	"""Change scene. Mirrors the same helper used in base_menu.gd, since
	Player does not inherit from BaseMenu and can't reuse it directly.
	The LED matrix's frame diffing keeps running across the scene change
	(LedMatrixManager is an autoload, unaffected by the scene swap), so it
	naturally sends only the pixels that actually differ between the old
	and new scene instead of a forced full resend."""
